...
function fillPath(pathTpl: string, params?: Record<string, string | number | boolean>): string {
...
async function generateCategoryFile(root: string, key: string, endpoints: any[]) {
  const filePath = path.join(root, `src/api/endpoints/${key}.ts`)
  await ensureDir(path.dirname(filePath))

  const lines: string[] = []
  lines.push(`// AUTO-GENERATED from data.ts. Do not edit.`)
  lines.push(`import { getClient } from '../client.js'`)
  ...
  for (const ep of endpoints) {
    ...
    const pathTpl = ep.path as string
    ...
    if (method === 'GET') {
      lines.push(`  return getClient().get<T>(p, args.query)`)
    } else if (method === 'POST') {
      lines.push(`  return getClient().post<T>(p, args.body)`)
    } else if (method === 'PUT') {
      lines.push(`  return getClient().put<T>(p, args.body)`)
    } else if (method === 'DELETE') {
      lines.push(`  return getClient().delete<T>(p)`)
    } else {
...
async function generateIndex(root: string, categories: any[]) {
...
async function generateDocs(root: string, categories: any[]) {
