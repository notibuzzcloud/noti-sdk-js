...
-d '{
	...
	"status": 1
      ...
      },
      {
        ...
        description: 'Envía una lista interactiva con opciones seleccionables.',
        headers: [
          { name: 'X-Api-Key', required: true, description: 'API key para autenticar', example: '<api_key>' }
        ],
        bodyExample: `{
...
-d '{
	...
	"supportAiCitations": [],
	"key": {
		"remoteJid": "51991649480@s.whatsapp.net",
		"fromMe": true,
		"id": "3EB0LIST1234567890ABCD"
	},
	"message": {
		"listMessage": {
			"title": "Simple Menu",
...
-H 'X-Api-Key: <api_key>'`,
  ...
  "success": true
      ...
      },
      {
        ...
        title: 'Archivar chat',
        description: 'Archiva el chat (lo mueve a archivados).',
        headers: [
          { name: 'X-Api-Key', required: true, description: 'API key para autenticar', example: '<api_key>' }
        ],
        pathParams: [
        ...
        request: `curl -X POST \
'{{BASE_URL}}/api/default/chats/111111111@c.us/archive' \
-H 'X-Api-Key: <api_key>'`,
        response: `{
  "success": true
}`
      },
      {
        ...
        description: 'Quita el chat de archivados.',
        headers: [
          { name: 'X-Api-Key', required: true, description: 'API key para autenticar', example: '<api_key>' }
        ],
        pathParams: [
        ...
        request: `curl -X POST \
'{{BASE_URL}}/api/default/chats/111111111@c.us/unarchive' \
-H 'X-Api-Key: <api_key>'`,
        response: `{
  "success": true
}`
      },
      {
        ...
        description: 'Marca el chat como no leído.',
        headers: [
          { name: 'X-Api-Key', required: true, description: 'API key para autenticar', example: '<api_key>' }
        ],
        pathParams: [
        ...
        request: `curl -X POST \
'{{BASE_URL}}/api/default/chats/111111111@c.us/unread' \
-H 'X-Api-Key: <api_key>'`,
        response: `{
  "success": true
}`
